...
function Photo({ product, large }) { const uri=product?.foto_url?.startsWith("http")?product.foto_url:product?.foto_url?`${API_URL}${product.foto_url}`:null; return uri ? <Image source={{ uri }} style={[s.photo, large && { height: 200 }]} /> : <View style={[s.photo, s.placeholder, large && { height: 200 }]}><Text style={s.placeholderText}>Foto</Text></View>; }
function Shell({ title, subtitle, children, logout, offline }) { return <SafeAreaView style={s.root}><View style={s.headerWrap}><View style={s.top}><View style={{flex:1}}><Text style={s.headerEyebrow}>JASTIP KAMPUS</Text><Text style={s.title}>{title}</Text>{subtitle?<Text style={s.headerSub}>{subtitle}</Text>:null}</View>{logout?<Pressable onPress={logout} style={s.logoutButton}><Text style={s.logoutText}>Keluar</Text></Pressable>:null}</View></View>{offline?<View style={s.offlineBanner}><Text style={s.offlineText}>⚠ Mode offline</Text></View>:null}<ScrollView contentContainerStyle={s.content}>{children}</ScrollView></SafeAreaView>; }
...
function Penitip({ token, user, offline, logout }) {
  ...
  let content;
  if(tab==="Sesi")content=<>{products.length?products.map(x=>{const toko=storeMap[x.session.store_id];return<Pressable key={`${x.session.id}-${x.id}`} onPress={()=>{setChosen(x);setTab("Detail")}} style={s.gofoodCard}><Photo product={x}/><View style={s.gofoodInfo}><Text style={s.gofoodName}>{x.nama}</Text><Text style={s.gofoodStore}>🏪 {x.session.store_name}</Text>{toko?.alamat?<Text style={s.gofoodAddr}>📍 {toko.alamat}</Text>:null}<View style={s.gofoodRow}><Text style={s.gofoodPrice}>{money(x.harga)}/{x.satuan}</Text><View style={s.gofoodBadge}><Text style={s.gofoodBadgeText}>Jasa {money(x.session.biaya_jasa_per_unit)}</Text></View></View><Text style={s.gofoodCap}>Sisa {x.session.kapasitas_tersisa} slot</Text></View></Pressable>}):<Card><Text style={s.muted}>Belum ada sesi aktif.</Text></Card>}</>;
