...
function Penjastip({ token, user, offline, logout }) {
  ...
  return <Shell title="Jastip Kampus · Penjastip" logout={logout} offline={offline}>{error ? <Text style={s.error}>{error}</Text> : null}{busy ? <ActivityIndicator color={C.blue} /> : null}
...
function Penitip({ token, user, offline, logout }) {
  const tabs=["Sesi","Detail","Tawaran & Bayar","Tracking","Riwayat"];const[tab,setTab]=useState("Sesi");const[sessions,setSessions]=useState([]);const[orders,setOrders]=useState([]);const[storeMap,setStoreMap]=useState({});const[chosen,setChosen]=useState(null);const[qty,setQty]=useState("5");const[variant,setVariant]=useState("");const[note,setNote]=useState("");const[offer,setOffer]=useState("");const[message,setMessage]=useState("");const[events,setEvents]=useState([]);const[payments,setPayments]=useState({});const[historyTracking,setHistoryTracking]=useState({});const[revisions,setRevisions]=useState({});const[busy,setBusy]=useState(false);
  async function refresh(){const[a,b,s]=await Promise.all([api.sessions(),api.myTitipan(token),api.stores().catch(()=>({stores:[]}))]),history=b.titipan||[];const map=Object.fromEntries((s.stores||[]).map(x=>[x.id,x]));const paymentEntries=await Promise.all(history.map(async x=>[x.id,await api.payment(x.id,token).catch(()=>null)]));const trackingEntries=await Promise.all(history.map(async x=>[x.id,(await api.tracking(x.id,token).catch(()=>({events:[]}))).events||[]]));const paymentMap=Object.fromEntries(paymentEntries),trackingMap=Object.fromEntries(trackingEntries);setSessions(a.sessions||[]);setOrders(history);setStoreMap(map);setPayments(paymentMap);setHistoryTracking(trackingMap);await storage.saveCache({publicSessions:a.sessions||[],history,owner:user.id,payments:paymentMap,historyTracking:trackingMap,storeMap:map})}
  useEffect(()=>{storage.loadCache().then(x=>{if(x?.value?.owner===user.id){setSessions(x.value.publicSessions||[]);setOrders(x.value.history||[]);setStoreMap(x.value.storeMap||{});setPayments(x.value.payments||{});setHistoryTracking(x.value.historyTracking||{})}});if(!offline)refresh().catch(e=>setMessage(e.message))},[offline]);
  ...
  let content;
  if(tab==="Sesi")content=<>{products.length?products.map(x=>{const toko=storeMap[x.session.store_id];return<Pressable key={`${x.session.id}-${x.id}`} onPress={()=>{setChosen(x);setTab("Detail")}} style={s.catalog}><Photo product={x} large/><Text style={s.cardTitle}>{x.nama}</Text><Text style={s.muted}>{x.session.store_name} · {money(x.harga)}/{x.satuan}</Text>{toko?.alamat?<Text style={s.muted}>📍 {toko.alamat}</Text>:null}<Text style={s.muted}>Sisa kapasitas {x.session.kapasitas_tersisa} · jasa {money(x.session.biaya_jasa_per_unit)}/unit</Text></Pressable>}):<Card><Text style={s.muted}>Belum ada sesi aktif.</Text></Card>}</>;
  if(tab==="Detail")content=chosen?<Card title={chosen.nama}><Photo product={chosen} large/><Text style={s.muted}>🏪 {chosen.session.store_name}</Text>{storeMap[chosen.session.store_id]?.alamat?<Text style={s.muted}>📍 {storeMap[chosen.session.store_id].alamat}</Text>:null}<Text style={s.muted}>⏰ Deadline {new Date(chosen.session.batas_waktu).toLocaleString("id-ID")}</Text><Input label="Jumlah (min. 5)" keyboardType="numeric" value={qty} onChangeText={setQty} placeholder="Ketik jumlah..."/><View style={s.wrap}>{[5,10,15,20,25,30].map(n=><Pressable key={n} onPress={()=>setQty(String(n))} style={[s.chip,qty===String(n)&&s.chipOn]}><Text style={qty===String(n)?s.chipTextOn:s.chipText}>{n}</Text></Pressable>)}</View><Input label="Varian" value={variant} onChangeText={setVariant}/><Input label="Catatan" value={note} onChangeText={setNote}/><Button title="Titip langsung" onPress={()=>create("langsung")}/><Input label="Tawaran jasa per unit (Rp)" keyboardType="numeric" value={offer} onChangeText={setOffer} placeholder="Ketik nominal..."/><View style={s.wrap}>{[1000,2000,3000,5000,7500,10000].map(n=><Pressable key={n} onPress={()=>setOffer(String(n))} style={[s.chip,offer===String(n)&&s.chipOn]}><Text style={offer===String(n)?s.chipTextOn:s.chipText}>{money(n)}</Text></Pressable>)}</View><Button secondary title="Ajukan tawaran" onPress={()=>create("tawar")}/></Card>:<Card><Text>Pilih produk dari daftar sesi.</Text></Card>;
